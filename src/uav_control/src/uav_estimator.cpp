...
UAV_estimator::UAV_estimator(ros::NodeHandle &nh): nh(nh)
{
    ...
    nh.param<int>("control/location_source", location_source, px_uav_msgs::UAVState::GPS);
    // 【参数】最大安全速度
    ...
    // 根据设定的定位来源订阅不同的定位数据
    if (location_source == px_uav_msgs::UAVState::MOCAP)
    {
        // 【订阅】mocap估计位置
        mocap_sub = nh.subscribe<geometry_msgs::PoseStamped>("/vrpn_client_node" + uav_name + "/pose", 1, &UAV_estimator::mocap_cb, this);
    }
    else if (location_source == px_uav_msgs::UAVState::VINS)
    {
        // 【订阅】VINS估计位置
        vins_sub = nh.subscribe<geometry_msgs::PoseStamped>("/vins_estimator/imu_propagate", 1, &UAV_estimator::vins_cb, this);
    }
    else if (location_source == px_uav_msgs::UAVState::T265)
    {
        // 【订阅】T265估计位置
        t265_sub = nh.subscribe<nav_msgs::Odometry>("/t265/odom/sample", 1, &UAV_estimator::t265_cb, this);
    }
    else if (location_source == px_uav_msgs::UAVState::OPTICAL_FLOW)
    {
        // 光流定位直连飞控，飞控积分获得位置，在px_uav中并不做操作
    }
    else if (location_source == px_uav_msgs::UAVState::viobot)
    {
        // 【订阅】viobot估计位置
        viobot_sub = nh.subscribe<nav_msgs::Odometry>("/pr_loop/odometry_rect", 1, &UAV_estimator::viobot_cb, this);
    }
    else if (location_source == px_uav_msgs::UAVState::MID360)
    {
        // 【订阅】MID360估计位置    
        mid360_sub = nh.subscribe<nav_msgs::Odometry>("/Odometry", 1, &UAV_estimator::mid360_cb, this);
    }
    else if (location_source == px_uav_msgs::UAVState::GAZEBO)
    {
        // 【订阅】gazebo仿真真值
        gazebo_sub = nh.subscribe<nav_msgs::Odometry>(uav_name + "/px_uav/ground_truth", 1, &UAV_estimator::gazebo_cb, this);
    }
    else if (location_source == px_uav_msgs::UAVState::FAKE_ODOM)
    {
        // 【订阅】fake odom
        fake_odom_sub = nh.subscribe<nav_msgs::Odometry>(uav_name + "/px_uav/fake_odom", 10, &UAV_estimator::fake_odom_cb, this);
    }
    else if (location_source == px_uav_msgs::UAVState::GPS || location_source == px_uav_msgs::UAVState::RTK)
    {
        // 【订阅】GPS状态，来自飞控
        ...
        set_local_pose_offset_sub = nh.subscribe<px_uav_msgs::GPSData>(uav_name + "/px_uav/set_local_offset_pose", 1, &UAV_estimator::set_local_pose_offset_cb, this);
        // 【发布】ENU坐标系下的位置偏移量
        local_pose_offset_pub = nh.advertise<px_uav_msgs::OffsetPose>(uav_name + "/px_uav/offset_pose", 1);
        // 【订阅】GPS卫星数量
    ...
    else if (location_source == px_uav_msgs::UAVState::UWB)
    {
        // 【订阅】UWB
        uwb_sub = nh.subscribe<px_uav_msgs::LinktrackNodeframe2>("/nlink_linktrack_nodeframe2", 10, &UAV_estimator::uwb_cb, this);
    }
    else if (location_source == px_uav_msgs::UAVState::BSA_SLAM)
    {
        // 【订阅】BSA_SLAM估计位置
    ...
    {
        text_info.MessageType = px_uav_msgs::TextInfo::WARN;
    ...
    // 【订阅】地面站修改ROS参数
    ros_param_set_sub = nh.subscribe<px_uav_msgs::ParamSettings>(uav_name + "/px_uav/param_settings", 1, &UAV_estimator::param_set_cb, this);

    // 【发布】无人机状态合集,包括位置\速度\姿态\模式等,供上层节点使用
    uav_state_pub = nh.advertise<px_uav_msgs::UAVState>(uav_name + "/px_uav/state", 1);
    ...
    uav_odom_pub = nh.advertise<nav_msgs::Odometry>(uav_name + "/px_uav/odom", 1);

    // 【发布】相机里程计,主要用于RVIZ显示
    camera_odom_pub = nh.advertise<nav_msgs::Odometry>(uav_name + "/px_uav/camera_odom", 1);

    // 【发布】无人机运动轨迹,主要用于RVIZ显示
    uav_trajectory_pub = nh.advertise<nav_msgs::Path>(uav_name + "/px_uav/trajectory", 1);

    // 【发布】无人机位置(带图标),用于RVIZ显示
    uav_mesh_pub = nh.advertise<visualization_msgs::Marker>(uav_name + "/px_uav/uav_mesh", 1);

    // 【发布】运行状态信息(-> 通信节点 -> 地面站)
    ground_station_info_pub = nh.advertise<px_uav_msgs::TextInfo>("/uav" + std::to_string(uav_id) + "/px_uav/text_info", 1);

    if (location_source == px_uav_msgs::UAVState::MOCAP || location_source == px_uav_msgs::UAVState::BSA_SLAM || location_source == px_uav_msgs::UAVState::T265 || location_source == px_uav_msgs::UAVState::viobot || location_source == px_uav_msgs::UAVState::GAZEBO || location_source == px_uav_msgs::UAVState::UWB|| location_source == px_uav_msgs::UAVState::VINS || location_source == px_uav_msgs::UAVState::MID360)
    {
    ...
    uav_state.gps_status = px_uav_msgs::UAVState::GPS_FIX_TYPE_NO_GPS;
    uav_state.gps_num = 0;
...
{
    if (location_source == px_uav_msgs::UAVState::GAZEBO)
    {
        vision_pose = gazebo_pose;
    }
    else if (location_source == px_uav_msgs::UAVState::MOCAP)
    {
    ...
    if (location_source == px_uav_msgs::UAVState::MID360) {
...
void UAV_estimator::timercb_rviz(const ros::TimerEvent &e)
{
    ...
    meshROS.color.g = 0.0;
    meshROS.color.b = 1.0;
    meshROS.mesh_resource = std::string("package://px_uav_uav_control/meshes/hummingbird.mesh");
    ...
    if(location_source == px_uav_msgs::UAVState::T265)
    {
        //  |----坐标系相对信息设置  偏移量  无人机相对于世界坐标系的坐标
...
void UAV_estimator::uwb_cb(const px_uav_msgs::LinktrackNodeframe2::ConstPtr &msg)
{
...
void UAV_estimator::check_uav_state()
{
    ...
    {
        if(location_source == px_uav_msgs::UAVState::GPS)
        {
            text_info.MessageType = px_uav_msgs::TextInfo::WARN;
        ...
        if(location_source == px_uav_msgs::UAVState::RTK)
        {
            text_info.MessageType = px_uav_msgs::TextInfo::WARN;
...
int UAV_estimator::check_uav_odom()
{
    ...
    if (location_source == px_uav_msgs::UAVState::GAZEBO && (time_now - get_gazebo_stamp).toSec() > GAZEBO_TIMEOUT)
    {
        return 1;
    }
    else if (location_source == px_uav_msgs::UAVState::MOCAP && (time_now - get_mocap_stamp).toSec() > MOCAP_TIMEOUT)
    {
        return 1;
    }
    else if (location_source == px_uav_msgs::UAVState::T265 && (time_now - get_t265_stamp).toSec() > T265_TIMEOUT)
    {
        return 1;
    }
    else if (location_source == px_uav_msgs::UAVState::viobot && (time_now - get_viobot_stamp).toSec() > VIOBOT_TIMEOUT)
    {
        return 1;
    }
    else if (location_source == px_uav_msgs::UAVState::MID360 && (time_now - get_mid360_stamp).toSec() > MID360_TIMEOUT)
    {
        return 1;
    }
    // UWB
    else if (location_source == px_uav_msgs::UAVState::UWB && (time_now - get_uwb_stamp).toSec() > UWB_TIMEOUT)
    {
        return 1;
    }
    else if (location_source == px_uav_msgs::UAVState::VINS && (time_now - get_vins_stamp).toSec() > VINS_TIMEOUT)
    {
        return 1;
    }
    else if (location_source == px_uav_msgs::UAVState::BSA_SLAM && (time_now - get_BSA_SLAM_stamp).toSec() > BSA_SLAM_TIMEOUT)
    {
        return 1;
    }
    else if ((location_source == px_uav_msgs::UAVState::GPS || location_source == px_uav_msgs::UAVState::RTK) && (time_now - get_gps_stamp).toSec() > GPS_TIMEOUT)
    {
        return 7;
    }
    ...
    if (location_source == px_uav_msgs::UAVState::MID360 && covariance_error)
    {
        return 11;
    }
    ...
    if (location_source == px_uav_msgs::UAVState::GPS)
    {
        if (uav_state.gps_status < px_uav_msgs::UAVState::GPS_FIX_TYPE_3D_FIX)
        {
            return 4;
        }
        else if(uav_state.gps_status > px_uav_msgs::UAVState::GPS_FIX_TYPE_3D_FIX)
        {
            return 5;
        }
    }
    ...
    if (location_source == px_uav_msgs::UAVState::RTK)
    {
        ...
        else if (uav_state.gps_status <= px_uav_msgs::UAVState::GPS_FIX_TYPE_RTK_FLOATR)
        {
            return 5;
        }
    }
...
void UAV_estimator::printf_uav_state()
{
    ...
    {
    case px_uav_msgs::UAVState::MOCAP:
        ...
        break;
    case px_uav_msgs::UAVState::T265:
        ...
        break;
    case px_uav_msgs::UAVState::OPTICAL_FLOW:
        ...
        break;
    case px_uav_msgs::UAVState::viobot:
        ...
        break;
    case px_uav_msgs::UAVState::MID360:
        ...
        break;
    case px_uav_msgs::UAVState::GAZEBO:
        ...
        break;
    case px_uav_msgs::UAVState::FAKE_ODOM:
        cout << GREEN << "Location: [ FAKE_ODOM ] " << TAIL << endl;
        break;
    case px_uav_msgs::UAVState::GPS:
        cout << GREEN << "Location: [ GPS ] " << TAIL;
        printf_gps_status();
        break;
    case px_uav_msgs::UAVState::RTK:
        cout << GREEN << "Location: [ RTK ] " << TAIL;
        printf_gps_status();
        break;
    case px_uav_msgs::UAVState::UWB:
        ...
        break;
    case px_uav_msgs::UAVState::VINS:
        ...
        break;
    case px_uav_msgs::UAVState::BSA_SLAM:
...
void UAV_estimator::printf_gps_status()
{
    ...
    if (location_source == px_uav_msgs::UAVState::GPS)
    {
        ...
        {
        case px_uav_msgs::UAVState::GPS_FIX_TYPE_NO_GPS:
            cout << RED << " [GPS_FIX_TYPE_NO_GPS] " << TAIL << endl;
            break;
        case px_uav_msgs::UAVState::GPS_FIX_TYPE_NO_FIX:
            cout << RED << " [GPS_FIX_TYPE_NO_FIX] " << TAIL << endl;
            break;
        case px_uav_msgs::UAVState::GPS_FIX_TYPE_2D_FIX:
            cout << YELLOW << " [GPS_FIX_TYPE_2D_FIX] " << TAIL << endl;
            break;
        case px_uav_msgs::UAVState::GPS_FIX_TYPE_3D_FIX:
    ...
    if (location_source == px_uav_msgs::UAVState::RTK)
    {
        ...
        case px_uav_msgs::UAVState::GPS_FIX_TYPE_3D_FIX:
            ...
            break;
        case px_uav_msgs::UAVState::GPS_FIX_TYPE_DGPS:
            cout << YELLOW << " [GPS_FIX_TYPE_DGPS] " << TAIL << endl;
            break;
        case px_uav_msgs::UAVState::GPS_FIX_TYPE_RTK_FLOATR:
            cout << YELLOW << " [GPS_FIX_TYPE_RTK_FLOATR] " << TAIL << endl;
            break;
        case px_uav_msgs::UAVState::GPS_FIX_TYPE_RTK_FIXEDR:
...
void UAV_estimator::printf_param()
{
    ...
    else if (location_source == px_uav_msgs::UAVState::GPS)
    {
        cout << GREEN << "location_source: [GPS] " << TAIL << endl;
    }
    else if (location_source == px_uav_msgs::UAVState::RTK)
    {
...
void UAV_estimator::set_local_pose_offset_cb(const px_uav_msgs::GPSData::ConstPtr &msg)
{
...
void UAV_estimator::param_set_cb(const px_uav_msgs::ParamSettings::ConstPtr &msg)
{
    try
    {
