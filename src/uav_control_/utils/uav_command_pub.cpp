...
px_uav_msgs::UAVCommand agent_command;
px_uav_msgs::TextInfo text_info;
std::vector<geometry_msgs::PoseStamped> posehistory_vector_;
std_msgs::Bool stop_control_state;

// 如果要使用地面站px_uavGround控制，需要将此值改为true，否则改为false
...
void uav_control_state_cb(const px_uav_msgs::UAVControlState::ConstPtr &msg)
{
...
int main(int argc, char **argv)
{
    ...
    //【订阅】状态信息
    ros::Subscriber uav_state_sub = nh.subscribe<px_uav_msgs::UAVState>(uav_name + "/px_uav/state", 1, uav_state_cb);

    //【订阅】无人机控制信息
    ros::Subscriber uav_contorl_state_sub = nh.subscribe<px_uav_msgs::UAVControlState>(uav_name + "/px_uav/control_state", 1, uav_control_state_cb);

    //【发布】UAVCommand
    ros::Publisher ref_trajectory_pub = nh.advertise<nav_msgs::Path>(uav_name + "/px_uav/reference_trajectory", 10);

    //【发布】UAVCommand
    ros::Publisher uav_command_pub = nh.advertise<px_uav_msgs::UAVCommand>(uav_name + "/px_uav/command", 1);

    //【订阅】无人机停止控制状态
    ros::Subscriber stop_control_state_sub = nh.subscribe<std_msgs::Bool>(uav_name + "/px_uav/stop_control_state", 1, stop_control_state_cb);

    //【发布】UAVCommand
    ros::Publisher text_info_pub = nh.advertise<px_uav_msgs::TextInfo>(uav_name + "/px_uav/text_info", 1);
    ...
    agent_command.Agent_CMD = px_uav_msgs::UAVCommand::Move;
    agent_command.Move_mode = px_uav_msgs::UAVCommand::XYZ_POS;
    ...
    while (ros::ok())
    {
        ...
        if (uav_control_state.control_state != px_uav_msgs::UAVControlState::COMMAND_CONTROL)
        {
        ...
        case 2:
            ...
            agent_command.header.stamp = ros::Time::now();
            agent_command.Agent_CMD = px_uav_msgs::UAVCommand::Move;
            agent_command.Move_mode = px_uav_msgs::UAVCommand::XYZ_POS_BODY;
            agent_command.position_ref[0] = state_desired[0];
            agent_command.position_ref[1] = state_desired[1];
            agent_command.position_ref[2] = state_desired[2];
        ...
        case 3:
            ...
            agent_command.header.stamp = ros::Time::now();
            agent_command.Agent_CMD = px_uav_msgs::UAVCommand::Current_Pos_Hover;
            uav_command_pub.publish(agent_command);
            break;
        ...
        case 4:
            ...
            agent_command.header.stamp = ros::Time::now();
            agent_command.Agent_CMD = px_uav_msgs::UAVCommand::Land;
            uav_command_pub.publish(agent_command);
        ...
        case 5:
            ...
            while (time_trajectory < trajectory_total_time)
            {
                ...
                agent_command.header.stamp = ros::Time::now();
                agent_command.Agent_CMD = px_uav_msgs::UAVCommand::Move;
                agent_command.Move_mode = px_uav_msgs::UAVCommand::TRAJECTORY;
                uav_command_pub.publish(agent_command);
        ...
        case 6:
            ...
            agent_command.header.stamp = ros::Time::now();
            agent_command.Agent_CMD = px_uav_msgs::UAVCommand::Move;
            agent_command.Move_mode = px_uav_msgs::UAVCommand::XYZ_VEL_BODY;
